...
const buildBreadcrumb = () => {
  ...
  const setupRetraissanceDropdown = () => {
    ...
    toggle.textContent = 'Retraissance Γû╛';
  ...
  const initMediaLayout = async () => {
    ...
    const ensureMediaControls = () => {
      ...
      mediaNodes.forEach(node => {
        ...
        ensureMoveBtn('inline-move-up', 'Γåæ', 'Move media up', 'up');
        ensureMoveBtn('inline-move-down', 'Γåô', 'Move media down', 'down');
    ...
    mkFmt('ΓÇó', 'Toggle bullet list (Ctrl+Shift+8)', 'insertUnorderedList');
    ...
    const setEditable = (on) => {
      ...
      const attachControls = (el, { allowMove = false, pairWithNext = false } = {}) => {
        ...
        ensureBtn('inline-move-up', 'Γåæ', 'Move up', (e) => { e.stopPropagation(); movePair('up'); });
        ensureBtn('inline-move-down', 'Γåô', 'Move down', (e) => { e.stopPropagation(); movePair('down'); });
      };
      ...
      if (!on) {
        ...
        mediaNodes.forEach(node => {
          ...
          ensureAlignBtn('inline-align-left', 'ΓåÉ', 'left');
          ensureAlignBtn('inline-align-center', 'ΓÇó', 'center');
          ensureAlignBtn('inline-align-right', 'ΓåÆ', 'right');
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!readerView) {
      ...
      // In reader mode, hide all editor controls and ensure view-only state.
